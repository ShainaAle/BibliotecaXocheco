...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Catch the array of publishers. Format will be: $_POST['publishers'][id_publisher] = "New Name"
    $publishers_data = $_POST['publishers'] ?? [];
    $has_errors = false;

    foreach ($publishers_data as $id => $name) {
        $id_clean = (int)$id;
        $name_clean = mysqli_real_escape_string($conn, trim($name));

        // Only update if the name is not empty
        if (!empty($name_clean)) {
            $query_update = "UPDATE publishers SET name = '$name_clean' WHERE id_publisher = $id_clean";
    ...
    if (!$has_errors) {
        echo "<script>
                alert('¡Editoriales actualizadas correctamente!'); 
                window.location.href='../authors-publishers.php';
              </script>";
        exit();
    } else {
        $alert_message = "<div class='alert alert-danger mt-3'>Hubo un error al actualizar una o más editoriales, o se dejó un campo en blanco.</div>";
    }
}
...
// Fetch the current data of the selected publishers
$query_select = "SELECT id_publisher, name FROM publishers WHERE id_publisher IN ($ids_string)";
$result_publishers = mysqli_query($conn, $query_select);

// If no valid publishers were found with those IDs, redirect back
    ...
    <main class="w-100 m-auto mt-5" style="max-width: 600px; padding: 15px;">
        <form action="" method="POST" class="p-4 border rounded-3">
            <div style="text-align: center;">
                <h1 class="h3 mb-3 fw-normal">Actualizar Editoriales</h1>
                <p class="text-muted">Modifica los nombres de las editoriales seleccionadas.</p>
            </div>

            <?php echo $alert_message; ?>

            <div class="container-fluid p-0 mt-4">
                <?php while ($publisher = mysqli_fetch_assoc($result_publishers)) { ?>
                    <div class="row mb-3 align-items-center">
                        <div class="col-md-2 text-end text-muted">
                            <small>ID: <?php echo $publisher['id_publisher']; ?></small>
                        </div>
                        <div class="col-md-10 form-floating">
                            <input type="text" class="form-control" 
                                   id="publisher_<?php echo $publisher['id_publisher']; ?>" 
                                   name="publishers[<?php echo $publisher['id_publisher']; ?>]" 
                                   value="<?php echo htmlspecialchars($publisher['name']); ?>" required>
                            <label for="publisher_<?php echo $publisher['id_publisher']; ?>" class="ms-2">Nombre de la Editorial</label>
                        </div>
                    </div>
                <?php } ?>
            </div>
